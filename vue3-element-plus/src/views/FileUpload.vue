<template>
  <div class="mainDiv">
    ...
    <UploadComponent
      v-show="false"
      @send-to-parent="handleChildMessage"
      ref="UploadComponent"
    />
  </div>
</template>
...
import { get_filelist, del_file } from '@/api/nodeApi'
...
export default {
  ...
  methods: {
    async fetchData () {
      const response = await get_filelist()
    ...
    // 上传组件
    handle_file_upload () {
      this.$refs.UploadComponent.dialogFormVisible = true
    },
    // 回传
